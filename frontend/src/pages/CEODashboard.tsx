...
export default function CEODashboard() {
  ...
  const [siteFilter, setSiteFilter] = useState<number | string | ''>('')
  const [exporting, setExporting] = useState<string | null>(null)

  const siteId = siteFilter === '' ? undefined : siteFilter

  const fetchActivities = useCallback((sid?: number | string) => {
    dashboard.getActivities(5, sid).then((r) => setActivities(Array.isArray((r as { activities?: unknown[] })?.activities) ? (r as { activities: unknown[] }).activities : [])).catch(() => {})
  }, [])

  useEffect(() => {
    setLoading(true)
    setError(null)
    Promise.all([dashboard.get(period, siteId), reports.get(period).catch(() => null)])
      .then(([d, r]) => {
        setData(d)
        setReportsData(r)
      })
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => fetchActivities(siteId), 30000)
        ...
        {data?.sites?.length > 0 && (
          <div className="ceo-dashboard__period">
            ...
            >
              ...
              {data.sites.map((s: { id: number; name: string }) => (
      ...
      <section className="ceo-dashboard__summary">
        ...
        <div className="ceo-dashboard__stats">
          <div className="ceo-stat">
            <span className="ceo-stat__value">{summary.total_sites}</span>
            <span className="ceo-stat__label">Sites</span>
          </div>
          <div className="ceo-stat">
            <span className="ceo-stat__value">{summary.total_service_requests}</span>
            <span className="ceo-stat__label">Service requests</span>
          </div>
          <div className="ceo-stat ceo-stat--success">
            <span className="ceo-stat__value">{summary.completed}</span>
            <span className="ceo-stat__label">Completed</span>
          </div>
          <div className="ceo-stat ceo-stat--revenue">
            <span className="ceo-stat__value ceo-stat__value--revenue">{formatCurrency(summary.total_revenue)}</span>
            <span className="ceo-stat__label">Revenue</span>
          </div>
          <div className="ceo-stat">
            <span className="ceo-stat__value">{summary.total_customers}</span>
            <span className="ceo-stat__label">Customers</span>
          </div>
          <div className="ceo-stat">
            <span className="ceo-stat__value">{summary.total_mechanics}</span>
            <span className="ceo-stat__label">Mechanics</span>
          </div>
          {summary.low_stock_count > 0 && (
            <div className="ceo-stat ceo-stat--warning">
              <span className="ceo-stat__value">{summary.low_stock_count}</span>
              <span className="ceo-stat__label">Low stock items</span>
            </div>
          )}
        </div>
      </section>
